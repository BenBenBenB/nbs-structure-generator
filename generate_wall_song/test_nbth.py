...
def christmas_tree(save_to_path, filename, width, block):
    t_structure = nbth.StructureBlocks()
    step_height = 3
    c1 = nbth.Coordinate(0, 0, 0)
    c2 = nbth.Coordinate(width - 1, step_height - 1, width - 1)
    count = 0
    while c2.x >= c1.x:
        pvol = nbth.Cuboid(c1, c2)
        count += t_structure.fill(pvol, block)
        count += t_structure.fill_frame(pvol, None)
        c1.add(nbth.Coordinate(1, step_height, 1))
        c2.add(nbth.Coordinate(-1, step_height, -1))
    t_structure.shift(nbth.Coordinate(0, 2, 0))
    c1.y = 0
    c2.y = 2
    pvol = nbth.Cuboid(c1, c2)
...
def test_fill(save_to_path, name, size: int, block1, block2):
    f_structure = nbth.StructureBlocks()
    c1 = nbth.Coordinate(0, 0, 0)
    c2 = nbth.Coordinate(size - 1, size - 1, size - 1)
    count = 0
    vol1 = nbth.Cuboid(c1, c2)
    vol2 = nbth.Cuboid(c1, c1)
    vol3 = nbth.Cuboid(c2, c2)
...
def test_clone(save_to_path, name, block1, block2):
    ...
    g_structure = nbth.StructureBlocks()
    g_structure.clone_structure(f_structure, c2 * 5)
    g_structure.set_block(c1, nbth.AIR_BLOCK)
